...
want_test(2, 'maybetest_outer not enabled');
maybe_test maybetest_outer  => sub {
    ok 1, "maybetest_inner";
};
...
testscript_ok("$snip2\n#line ".(__LINE__+2).<<'EOSCRIPT', 2, "timed_test");

want_test(1, 'timed_pass_outer',
    '-re'.'timed_pass_outer start: \\d+\.\\d+',
    '-re'.'timed_pass_outer done:  \\d+\.\\d+',
);
timed_test timed_pass_outer => sub {
    ok 1, 'timed_pass_inner';
};

want_test(0, 'timed_fail_outer',
    '-re'.'timed_fail_outer start: \\d+\.\\d+',
    fail_diag('timed_fail_inner', 0, __LINE__+6),
    '',
    fail_diag('timed_fail_outer', 1, __LINE__+5),
    '-re'.'timed_fail_outer done:  \\d+\.\\d+',
);
...
testscript_ok("$snip3\n#line ".(__LINE__+2).<<'EOSCRIPT', 4, "nopathc");

next_test_nopathchange();
want_test(1, 'pass_nomess_outer');
test pass_nomess_outer => sub {
    ok 1, 'pass_nomess_inner';
};

next_test_nopathchange();
want_test(0, 'fail_nomess_outer',
    fail_diag('fail_nomess_inner', 0, __LINE__+5),
    '',
...
next_test_nopathchange();
want_test(0, 'failmess_nomess_outer',
    fail_diag('failmess_nomess_inner', 0, __LINE__+8),
    fail_diag('path not modified', 0),
    '-re^\\s*got:.*foo',
    '-re^\\s*expected:',
    '',
...
testscript_ok("$snip4\n#line ".(__LINE__+2).<<'EOSCRIPT', 3, "wwdebug");

delete $ENV{DEBUG};

want_test(1, 'passboth_debug_outer');
next_test_with_and_without_debug();
test passboth_debug_outer => sub {
    ok 1, 'passboth_debug_inner';
};

want_test(0, 'failboth_debug_outer',
    fail_diag('failboth_debug_inner', 0, __LINE__+7),
...
want_test(0, 'failone_debug_outer',
    fail_diag('failone_debug_inner', 0, __LINE__+6),
    '',
...
testscript_ok("$snip5\n#line ".(__LINE__+2).<<'EOSCRIPT', 2, "mytest");

local $ENV{PATH} = $ENV{PATH} . ":bar";

want_test(1, 'allpass_outer');
mytest allpass_outer => sub {
    ok 1, 'allpass_inner';
};

want_test(0, 'allfail_outer',
    fail_diag('allfail_inner', 0, __LINE__+12),
    fail_diag('path not modified', 0),
    '-re^\\s*got:.*:bar:foo\\W*$',
    '-re^\\s*expected:.*:bar\\W*$',
    fail_diag('allfail_inner', 0, __LINE__+8),
    fail_diag('path not modified', 0),
    '-re^\\s*got:.*:bar:foo:foo\\W*$',
    '-re^\\s*expected:.*:bar:foo\\W*$',
    '',
