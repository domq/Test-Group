...
code after the test() call. For this situation L<Test::Group> provides
C<$Test::Group::Level> to correct the line numbers in failed test
diagnostics. This is similar to the C<$Test::Builder::Level> variable
in L<Test::Builder>.
...
L<Test::Group> provides the function next_test_plugin() (not exported
by default) to install a plugin for the next test group.  A plugin is
a subroutine that sits in between Test::Group::test() and the
subroutine reference passed to it.
...
Another example - the following plugin runs the test group twice, with and
without a DEBUG environment variable set:
